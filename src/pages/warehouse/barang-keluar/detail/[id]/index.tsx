...
import { Edit, Calendar, Package, MapPin, User, FileText, UserCircle, DollarSign } from "lucide-react";

const BarangKeluarDetailPage: React.FC = () => {
    const params = (useRRParams() as unknown as { id?: string }) ?? {};
    const id = params?.id ?? "";
    const navigate = useNavigate();

    const { data, isLoading, isError } = useBarangKeluarById({ id });
    const barangKeluar = data?.data;
                ...
                <div className="flex flex-col gap-4">
                    <div className="rounded-xl shadow-sm p-6 bg-white">
                        ...
                        <div className="bg-red-50 border border-red-200 text-red-600 rounded-xl p-4">
                            Terjadi kesalahan saat memuat detail barang keluar.
                        </div>
                    </div>
                </div>
            </Dashboard>
    ...
    const noteType = typeof barangKeluar.note_type_id === 'object' ? barangKeluar.note_type_id?.name : '-';
    const customer = typeof barangKeluar.customer_id === 'object' ? barangKeluar.customer_id : null;
    const product = typeof barangKeluar.product_id === 'object' ? barangKeluar.product_id : null;
    const handledBy = typeof barangKeluar.handled_by === 'object' ? barangKeluar.handled_by : null;
                ...
                <div className="rounded-xl shadow-sm p-6 bg-white">
                    <div className="flex justify-between items-start">
                        ...
                        <div className="flex gap-2">
                            <Button
                                variant={"gradien"}
                                icon={<Edit />}
                                text="Edit Barang Keluar"
                                onClick={() =>
                                    navigate(`/warehouse/barang-keluar/update/${barangKeluar._id}` as Parameters<typeof navigate>[0])
                                }
                            />
                        </div>
                    </div>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            <StatusPill label={`Note: ${barangKeluar.note_number}`} color="blue" width="auto" />
                            <StatusPill label={`Tanggal: ${formatDateLong(barangKeluar.date)}`} color="green" width="auto" />
                            <StatusPill label={`Qty: ${barangKeluar.qty_out} ${barangKeluar.unit_snapshot}`} color="purple" width="auto" />
                        </div>
                    </div>
                    ...
                    <div className="mt-8 bg-linear-to-br from-blue-50/50 to-purple-50/50 rounded-xl p-6 border border-blue-100/50">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <DetailItem
                                label="Tanggal"
                                value={formatDateLong(barangKeluar.date)}
                                icon={<Calendar className="w-4 h-4" />}
                            />
                            <DetailItem
                                label="Note Number"
                                value={barangKeluar.note_number}
                    ...
                    <div className="mt-6 bg-linear-to-br from-green-50/50 to-emerald-50/50 rounded-xl p-6 border border-green-100/50">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <DetailItem
                                label="Nama Produk"
                                value={barangKeluar.product_name_snapshot || (product?.product_name || "-")}
                            ...
                            <DetailItem
                                label="Quantity Out"
                                value={`${barangKeluar.qty_out} ${barangKeluar.unit_snapshot}`}
                                icon={<Package className="w-4 h-4" />}
                            />
                            <DetailItem
                                label="Unit"
                                value={barangKeluar.unit_snapshot}
                                icon={<Package className="w-4 h-4" />}
                            />
                            <DetailItem
                                label="HPP Snapshot"
                                value={formatRupiah(barangKeluar.hpp_snapshot)}
                                icon={<DollarSign className="w-4 h-4" />}
                            />
                            <DetailItem
                                label="Total HPP"
                                value={formatRupiah(barangKeluar.total_hpp)}
                                icon={<DollarSign className="w-4 h-4" />}
                            />
                        </div>
                    </div>
                    ...
                    <div className="mt-6 bg-linear-to-br from-indigo-50/50 to-violet-50/50 rounded-xl p-6 border border-indigo-100/50">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <DetailItem
                                ...
                                value={barangKeluar.additional_info || "-"}
                                icon={<FileText className="w-4 h-4" />}
                                fullWidth
                            />
                        </div>
                    ...
                    <div className="mt-6 bg-gray-50 rounded-xl p-6 border border-gray-200">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <DetailItem
                                label="Created At"
                                value={formatDateLong(barangKeluar.createdAt)}
                            />
                            <DetailItem
                                label="Updated At"
                                value={formatDateLong(barangKeluar.updatedAt)}
                            />
                            <DetailItem
                                label="ID"
                                value={barangKeluar._id}
                                fullWidth
                            />
                        </div>
                    </div>
